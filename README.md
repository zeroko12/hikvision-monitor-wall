...
- **低延迟**：ffmpeg 低延迟输入参数 + 1s 分片滑窗（仅保留 6 片），端到端延迟约 2~3 秒
- **视口按需取流**：进入视口的通道才启动转码，离开视口自动停流释放资源（手机上看只跑可见的几路）
- **容错**：启动限流防设备会话占满；流启动健康检查 + 前端超时自动重试，避免永久黑屏
- **通道管理**：卡片可拖拽排序、可隐藏通道，偏好保存在浏览器本地，刷新/重开自动恢复
...
- **个别通道长时间黑屏**：设备 RTSP 会话可能被占满（尤其 NVR 多路同时观看）。本版已加启动限流与健康检查，页面会自动重试；持续失败多为该路设备离线/密码不同。
- **画面卡顿/延迟大**：确认使用桌面浏览器；视口外通道会自动停流释放资源；单路延迟 2~3 秒为 HLS 正常水平。
